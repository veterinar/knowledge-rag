...
def test_c1_memory_mb_pair_not_gated_with_info(monkeypatch, capsys, tmp_path):
    # (0.10 -> 0.14 «MB», +40%) — имя из MEMORY_MB_BENCHES: нет fail, есть info
    code, out, err = _run(
        monkeypatch,
        capsys,
        tmp_path,
    ...
    assert "[FAIL]" not in err
    assert "[OK] No benchmarks regressed beyond threshold." in out
    assert "[INFO] memory-delta benchmarks (MB, gated by their own asserts + Pillar 3):" in out
    assert "test_bench_orchestrator_idle_rss" in out
    assert "0.10 -> 0.14 MB" in out
    assert "+40.0%" in out
...
def test_c2_summary_prints_memory_excluded_count(monkeypatch, capsys, tmp_path):
    ...
    assert "Memory-delta pairs excluded from gate: 2" in out
